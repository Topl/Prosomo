...
object ActorRefWrapper {
  ...
  def apply(actorRef:akka.actor.ActorRef)(implicit routerRef:ActorRefWrapper):ActorRefWrapper = {
    new ActorRefWrapper(actorRef = actorRef ,actorPath = actorRef.path,remote = false,inputRouterRef = routerRef.actorRef)
  }

  def apply(path:akka.actor.ActorPath)(implicit routerRef:ActorRefWrapper):ActorRefWrapper = {
    new ActorRefWrapper(actorPath = path, inputRouterRef = routerRef.actorRef, remote = true, actorRef = routerRef.actorRef )
  }
