...
class PeerManager(settings: Settings) extends Actor with ScorexLogging {
  ...
  if (peerDatabase.isEmpty()) {
    settings.knownPeers.foreach { address =>
      val peerInfo = PeerInfo(System.currentTimeMillis(), Some(settings.nodeNonce), Some("bootstrap"))
      ...
      if (peerDatabase.isBlacklisted(address)) {
        ...
        handshake.declaredAddress match {
          case Some(newAddress:InetSocketAddress) => {
            ...
            self ! AddOrUpdatePeer(newAddress, Some(handshake.nodeNonce), Some("declared"))
          }
          case _ =>
        }
