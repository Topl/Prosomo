...
The project can be compiled and run either in the scala build tool (sbt) console or staged for deployment to a server by running ‘sbt stage’ in the project directory.  Check our github for the latest release candidate.  A command line script for interacting with the simulation is provided in the project directory.  To execute the command line, run ./cmd.sh in a separate terminal while in the project directory.  This is used to pass commands to Prosomo via a file and can be used to queue commands at a later slot.  Just enter the desired slot number next to the command.  The command line input is useful for debugging, setting up specific network conditions, and manipulating repeated simulation runs in real time.

The project configuration can be altered in the src/main/resources/application.conf which lists the default values for Prosomo and Scorex.  Comments describe all the options and most should be left at their default value for a persistent test-net.  For local and private test-net experiments, the configuration can be modified at runtime with command line input values entered as arguments to the Prosomo executable.  The arguments must be HOCON formatted with no spaces surrounded by and input path, e.g. "prosomo input{params{yourOptionHere=yourOptionValue}}".  Local configuration files can also be read in from the project directory by entering the file name, e.g. "prosomo yourSettings.conf".

Prosomo includes a simple user interface that allows users to interact with the network, create keys, and issue transactions.  A list of peers is displayed that provides information about their stake and account balance.  Block activity is displayed in an embedded terminal window.  The simulation can also be run from command line without the user interface.
